...
void CPU::HandleInterrupts() {
  uint8_t IE = bus.Peek(0xFFFF);
  uint8_t IF = bus.Peek(0xFF0F);
  ...
  if (IME) {
    if (pending) {
      ...
      uint8_t curr_IF = bus.Peek(0xFF0F);
      if (pending & 0x01) { // VBlank
        PC = 0x0040;
        bus.SetIO(0x0F, curr_IF & 0xFE);
      } else if (pending & 0x02) { // LCD
        PC = 0x0048;
        bus.SetIO(0x0F, curr_IF & 0xFD);
      } else if (pending & 0x04) { // Timer
        PC = 0x0050;
        bus.SetIO(0x0F, curr_IF & 0xFB);
      } else if (pending & 0x08) { // Serial
        PC = 0x0058;
        bus.SetIO(0x0F, curr_IF & 0xF7);
      } else if (pending & 0x10) { // Joypad
        PC = 0x0060;
        bus.SetIO(0x0F, curr_IF & 0xEF);
      }
      bus.Tick(4);
    }
  }
